...
from models.multiobjILPmodel import MultiobjectiveILPmodel

from ILP_operations import ILPOperations
from ILP_operations import transformations as REFACTORINGS
...
class ILPEngine():
    ...
    def apply_defined_weighted_sum(self, refactoring: ILPOperations, files: ModelInstance, threshold: integer, weights: (w1,w2,w3)) -> Solution:
        """Apply weighted sum algorithm with specified weights to the given instance."""
        return refactoring.transform(files, threshold)

    def apply_succesive_weighted_sum(self, refactoring: FMRefactoring, files: ModelInstance, threshold: integer, subdivisions: integer) -> Solution:
    ...
    def apply_epsilon_constraint(self, refactoring_name: str) -> FMRefactoring:
    ...
    # def apply_biesbinsky(self, refactoring_name: str) -> FMRefactoring:
    ...
    def apply_TPA(self, refactoring_name: str) -> FMRefactoring:
        """Given the name of an operation class, return the instance class of the ."""
        modules = inspect.getmembers(REFACTORINGS)
        modules = filter(lambda x: inspect.ismodule(x[1]), modules)
        modules = [importlib.import_module(m[1].__name__) for m in modules]
