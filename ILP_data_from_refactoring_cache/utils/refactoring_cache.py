...
argument which belongs to data in the original dataset
...
Note that the input dataframe can be generated by the set_extractions_id function """
def get_extractions_including_given_columns(df: pd.DataFrame, columns_to_add: List[str]) -> pd.DataFrame:
    # Create a set of tuples (..., ...) to store columns
    extractions = set()
    ...
    for index in range(0, len(df)):
        ...
        tuple_to_add = (extraction, ) + tuple(row[c] for c in columns_to_add)
        extractions.add(tuple_to_add)

    # Move the extractions to a DataFrame
    result = pd.DataFrame(list(extractions), columns=['extraction'] + columns_to_add)

    # Sort the DataFrame by the first column
    result = result.sort_values(by=['extraction'], ascending=[True])
...
reduction (CCR) removed from the ancestor when removing a nested extraction. The CCR is computed using the input
dataframe.
...
Note that the first column can be generated by the set_extractions_id function """
