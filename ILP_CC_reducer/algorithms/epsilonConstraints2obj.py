...
class EpsilonConstraintAlgorithm2obj(Algorithm):
    ...
    def execute(data: dp.DataPortal, tau: int, objectives_list: list) -> None:
        ...
        if (result.solver.status == 'ok') and (result.solver.termination_condition == 'optimal'):
            ...
            multiobj_model.model.f2Constraint = pyo.Constraint(rule=lambda m: multiobj_model.second_obj_diff_constraint(m, obj2))
            ...
            while solution_found:
                ...
                algorithms_utils.modify_component(multiobj_model, 'lambda_value', pyo.Param(initialize=1/(f1z - u1), mutable=True))
                # lambd = 1/(f1z - u1)
                
                """ Solve {min f2(x) - lambda * l, subject to f1(x) + l = epsilon} """
                # min f2(x) - lambda * l
                algorithms_utils.modify_component(multiobj_model, 'obj', pyo.Objective(
                    rule=lambda m: multiobj_model.epsilonObjective_2obj(m, obj2)))
                # subject to f1(x) + l = epsilon
                algorithms_utils.modify_component(multiobj_model, 'epsilonConstraint', pyo.Constraint(
                    rule=lambda m: multiobj_model.epsilonConstraint_2obj(m,obj2)))
                # Solve
                concrete, result = algorithms_utils.concrete_and_solve_model(multiobj_model, data)
